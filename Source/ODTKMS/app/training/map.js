...
(function()
{
    ...
    var controllerId = 'trainingMap';

    angular.module('app').controller(controllerId,
        ['$scope', '$location',  '$routeParams','$window', 'common', 'config',
         'datacontext', 'model', 'uiGmapIsReady', trainingMap]);

    function trainingMap($scope, $location, $routeParams, $window, common, config,
        datacontext, model, uiGmapIsReady)
    {
        var vm = this;
        vm.title = 'NIDM Completed Trainings Map';
            ...
            {               
                for(var i = 0; i < vm.allTrainings.length; i++)
                {
                    ...
                    if(t.location)
                    {
                        ...
                        //var content = t.description;

                        var content = '<dl class="dl-horizontal"><dt>Title</dt><dd>' + t.name
                                    + '</dd><dt>City</dt><dd>' + t.city.name
                        ...
                        if(t.objective)
                            content += '<dt>Objective</dt><dd>' + t.objective + '</dd>';
       ...
       function getAll(forceRefresh) {
           ...
           var select = 'trainingId, name, objective, description, broaderTrainingArea, broaderTrainingArea.name, trainingLevel, startDate, endDate, city, city.name, location, status';
            var where = breeze.Predicate.create('trainingId', '!=', 0).and('status', '==', 'Completed');
