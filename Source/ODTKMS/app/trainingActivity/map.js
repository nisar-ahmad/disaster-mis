...
(function()
{
    ...
    var controllerId = 'trainingActivityMap';

    angular.module('app').controller(controllerId,
        ['$scope', '$location',  '$routeParams','$window', 'common', 'config',
         'datacontext', 'model', 'uiGmapIsReady', trainingActivityMap]);

    function trainingActivityMap($scope, $location, $routeParams, $window, common, config,
        datacontext, model, uiGmapIsReady)
    {
        var vm = this;
        vm.title = 'Mapping DRM Trainings';
        ...
        {
            if (isReady && vm.allTrainings.length > 0)
            {
                for(var i = 0; i < vm.allTrainings.length; i++)
                {
                    var t = vm.allTrainings[i];

                    if(t.location)
                    {
                        var latlngStr = t.location.split(" ", 2);
                        var lng = parseFloat(latlngStr[0]);
                        var lat = parseFloat(latlngStr[1]);               

                        var latLng = new google.maps.LatLng(lat, lng);

                        var content = '<dl class="dl-horizontal"><dt>Title</dt><dd>' + t.titleOfTraining
                                    + '</dd><dt>Location</dt><dd>' + t.district.name
                                    + '</dd><dt>Start Date</dt><dd>' + moment(t.startDate).format('DD MMM YYYY')
                                    + '</dd><dt>End Date</dt><dd>' + moment(t.endDate).format('DD MMM YYYY')
                                    + '</dd><dt>Broader Area</dt><dd>' + t.broaderTrainingArea.name;
       ...
       function getAll(forceRefresh) {
           ...
           var select = 'trainingActivityId, titleOfTraining, broaderTrainingArea, broaderTrainingArea.name, startDate, endDate, district, district.name, location';
           var where = breeze.Predicate.create('trainingActivityId', '!=', 0);

            return datacontext.trainingActivity.getAll(forceRefresh, select, where).then(function (data) {
                vm.allTrainings = data;
                    placeMarkers();
                    return data;
                }
            );
        ...
        function goToIndex()
        {
            $location.path('/trainingActivities');
        }
    }
